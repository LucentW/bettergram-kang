...
void MainWindow::psCreateTrayIcon() {
	...
	if (useAppIndicator) {
		...
		if (_trayMenu) {
			...
			if (iconFile.exists()) {
				QByteArray path = QFile::encodeName(iconFile.absoluteFilePath());
				_trayIndicator = Libs::app_indicator_new("Bettergram", path.constData(), APP_INDICATOR_CATEGORY_APPLICATION_STATUS);
				if (_trayIndicator) {
		...
		if (Libs::gdk_init_check(0, 0)) {
			...
			if (_trayMenu) {
				...
				if (_trayIcon) {
					...
					Libs::gtk_status_icon_set_title(_trayIcon, "Bettergram");
					Libs::gtk_status_icon_set_tooltip_text(_trayIcon, "Bettergram");
					Libs::gtk_status_icon_set_visible(_trayIcon, true);
				} else {
					useStatusIcon = false;
				}
